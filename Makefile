...
test:
	@if [ -f tests/test_*.py ]; then \
		...
		echo "⏭️  No tests found in tests/ - skipping coverage checks"; \
	fi
test-watch:
	pytest-watch tests/ -- --maxfail=1 -v
lint:
	@echo "🎨 Linting with pylint..."
	-pylint src/ --disable=fixme --exit-zero
